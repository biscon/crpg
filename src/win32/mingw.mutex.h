...
protected:
    void checkSetOwnerAfterLock()
    {
        ...
        {
            std::fprintf(stderr, "FATAL: Recursive locking of non-recursive mutex detected. Throwing engine exception\n");
            std::fflush(stderr);
    ...
    void checkSetOwnerBeforeUnlock()
    {
        ...
        if (mOwnerThread != self)
        {
            std::fprintf(stderr, "FATAL: Recursive unlocking of non-recursive mutex detected. Throwing engine exception\n");
            std::fflush(stderr);
            throw std::system_error(EDEADLK, std::generic_category());
        }
        mOwnerThread = 0;
